# -*- coding: utf-8 -*-
"""
Created on Tue Jun  5 08:43:59 2018
...
token=Tokenizer(num_words=2000)  #建立token,输入参数num_words=2000，也就是一个2000个单词的字典
...
x_train=sequence.pad_sequences(x_train_seq,maxlen=100)#截长补短，让每一个数字列表长度为100
...
#建立嵌入层，将数字列表转换成线性列表,转换成32维向量，每一项有100个数字
...
model.add(Dropout(0.35))        #添加Dropout层,避免过拟合，设置去取消权值的神经元比
...
display_test_SentimentDict(14)    #输出显示第几项数据
